<template>
  <div>
    <zdzywhPage
      ...
      @getAssociatedConnectorBox="getAssociatedConnectorBox"
    ...
    >
      ...
      <template v-slot:handle="slot">
        <el-button
          type="primary"
          size="mini"
          @click="openFiberInfo(true, slot.scope.row)"
          v-hasPermi="['system:b:update']"
        >编辑</el-button>
        <el-button
          type="primary"
          size="mini"
          @click="openFiberInfo(false, slot.scope.row)"
          v-hasPermi="['system:b:list']"
        >查看</el-button>
        <el-button
          type="danger"
          size="mini"
          @click="delData(slot.scope.row)"
          v-hasPermi="['system:b:remove']"
        >删除</el-button>
        <el-dropdown
    ...
    >
          ...
          <el-tabs v-model="fiberInfoBox.index" type="card">
            <el-tab-pane label="路由" name="0">
              <ul style="padding-bottom: 10px">
                <template
                  v-for="(item) in fiberInfoBox.data.topoList"
                >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button
          v-if="iframe == 'iframe'"
          :loading="createFiberLineLoading"
          type="primary"
    ...
    <el-dialog
      ...
      :title="editBox2.title"
      :modal="false"
      :close-on-click-modal="false"
      :visible.sync="editBox2.show"
      width="750px">
      <editData
        ref="child2"
        :modelObj="editBox2.modelObj"
        :dataObj="editBox2.data"
        :type="editBox2.editType"
        :dbName="'`ry-middledb`'"
      ></editData>
      <div slot="footer" class="dialog-footer">
    ...
    <el-dialog
      ...
      <div slot="footer" class="dialog-footer">
        <el-button
          :loading="creategTcconFiberlineRFiberseglineLoading"
          type="primary"
    ...
    <el-dialog
      v-dialogDrag
      :title="fiberLineRelation.title + '   关联一次线路'"
      top="5vh"
      :visible.sync="fiberLineRelation.show"
      v-if="fiberLineRelation.show"
      :modal="false"
      :close-on-click-modal="false"
      width="90%">
    ...
    <el-dialog
      v-dialogDrag
      custom-class="curdBox"
      :title="associatedConnectorBox.title + '   断点定位'"
      v-if="associatedConnectorBox.show"
      :visible.sync="associatedConnectorBox.show"
      :modal="false"
      :close-on-click-modal="false"
      width="90%">
      <fiberRFbrconnect
