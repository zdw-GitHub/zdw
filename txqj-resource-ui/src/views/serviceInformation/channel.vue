<template>
  <div class="commonWh">
    ...
    <channelTopo :parRow="editForm" ref="channelTopo"></channelTopo>
  </div>
</template>
...
import TTD from "@/views/serviceInformation//TTD";
import channelTopo from "@/views/serviceInformation/channelTopo.vue";
...
export default {
  ...
  components: {channelTopo, editData, seeData,TTD},
  data() {
    return {
    ...
    obtainChannelLinks() {
      ...
      queryTheBoundLinkBasedOnTheBusinessID({
        ...
        if (this.link.tableData.length) {
          ...
          this.$refs.TTD.llId = this.llId
        }else {
          this.llId = ''
          this.$refs.TTD.llId = ''
        }
        this.$refs.TTD.getTableData()
        this.link.loading = false
      })
    },
    openRouter (row) {
      this.editForm = row
      this.$refs.channelTopo.parRow = row
      this.$refs.channelTopo.lightPathBoxShow()
    },
    //编辑
    openEdit(row) {
      this.editForm = row
