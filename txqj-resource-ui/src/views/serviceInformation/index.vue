<template>
  <div class="serviceInformation">
    <div class="pageBox">
      <!--业务-->
      <zdzywhPage ref="zdzywhPage" v-show="stepNum == 0" :tableName="tableName"
        :operate="{ slot: 'handle', label: '操作', width: '250', }" :rightClickMenus="[
          { name: '复制路由', icon: 'el-icon-document-copy', functionName: 'copyRoute' },
          { name: '业务路由', icon: 'el-icon-upload2', functionName: 'openBuzRouter' },
        ]" @copyRoute="copyRoute" @openBuzRouter="openBuzRouter" :tabOperate="true">
        ...
        <template v-slot:handle="slot">
          ...
          <el-button size="mini" type="primary" @click="openInfo(true, slot.scope.row)"
            v-hasPermi="['system:b:update']">编辑</el-button>
          <el-button type="primary" size="mini" @click="openInfo(false, slot.scope.row)"
            v-hasPermi="['system:b:list']">查看</el-button>
          <el-button type="danger" size="mini" @click="delData(slot.scope)"
...
export default {
  ...
  mounted() {
    // 从路由 query 获取树节点数据
    const { treeNode } = this.$route.query;
    if (treeNode) {
      try {
  ...
  methods: {
    initData(treeNode) {
      ...
      this.$nextTick(() => {
        if (this.$refs.zdzywhPage) {
          // 如果节点数据中有 id，则按 ID 筛选
          if (treeNode.id) {
            this.$refs.zdzywhPage.searchForm.ID = treeNode.id;
          }
          // 刷新表格
          this.$refs.zdzywhPage.getTableData();
        }
    ...
    getTableDataBuz(treeNode) {
      if (treeNode) {
        this.initData(treeNode);
      } else {
        this.$refs.zdzywhPage.getTableData();
      }
    },
