<template>
  <div id="fiberCoreDetail">
    <el-row>
      <el-col :span="8">
        <el-table
          stripe
          border
          v-loading="detailsData.loading"
          height="75vh"
          :span-method="objectSpanMethod"
          :data="detailsData.data">
          <el-table-column
            v-if="detailsData.data[0]"
            prop="glqx"
            label="光缆纤芯"
            align="center"
            width="50"
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <el-input-number
                ...
                :min="1"
                :controls="false"
                v-if="scope.$index == detailsDataEditBox.index && detailsDataEditBox.show"
                v-model="detailsDataEditBox.gldqx"
              ></el-input-number>
              <span v-else>{{ scope.row.gldqx }}</span>
            </template>
          </el-table-column>
          <el-table-column
            label="操作"
            ...
            <template slot-scope="scope">
              <el-button
                size="mini"
                v-if="!(scope.$index == detailsDataEditBox.index && detailsDataEditBox.show)"
                @click="detailsDatatableEdit(scope.row, scope.$index)"
                icon="el-icon-edit"
              ></el-button>
              <el-button
                size="mini"
                v-if="scope.$index == detailsDataEditBox.index && detailsDataEditBox.show"
                @click="detailsEditCommit"
                icon="el-icon-check"
              ></el-button>
              <el-button
                size="mini"
                v-if="scope.$index == detailsDataEditBox.index && detailsDataEditBox.show"
                @click="detailsDataEditBox.show = false"
                icon="el-icon-close"
              ></el-button>
            </template>
          </el-table-column>
    ...
    <div class="seeDetailsBox" v-if="false">
      <p class="title">
        纤芯路由详情
...
import {
  ...
  formEditCommitApi,
  querylist,
  updataSgTcdevFiberSegLineRLineRB
...
export default {
  ...
  methods: {
        ...
        window.setTimeout(() => {
          ...
          for (let i = 0; i < this.data.LLlist.length; i++) {
            if (this.data.LLlist[i].fiberlineId == id) {
              ...
              let doc = document.getElementsByClassName('fibers')[0].getElementsByClassName('b-body')[segIndex].getElementsByClassName('glxx')[segLineIndex]
          ...
          let doc = document.getElementsByClassName('bottomFiber')[0].getElementsByClassName('b-body')[0].getElementsByClassName('glxx')[fiberLineIndex]
          let className = doc.getAttribute('class')
          doc.setAttribute('class', className + ' selectGlxx')
        }, 10)
      } else if (type == 'fiber') {
    ...
    getDetails() {
      ...
      querylist({parFiber: this.data.fiberObj.id}).then(res => {
        this.detailsData.data = res
        this.detailsData.loading = false
      })
    },
    rowKeyFunc(row) {
      return row.glqx + row.gld
    },
    ...
    detailsDatatableEdit(row, index) {
      this.detailsDataEditBox = {
        show: true,
        index: index,
        id: row.ID,
        gldqx: parseInt(row.gldqx),
        glqx: parseInt(row.glqx),
        gldId: row.gldId
      }
    },
    objectSpanMethod({row, column, rowIndex, columnIndex}) {
      if (columnIndex === 0) {
        const _row = this.flitterData(this.detailsData.data).one[rowIndex]
    ...
    detailsEditCommit() {
      updataSgTcdevFiberSegLineRLineRB({
        id: this.detailsDataEditBox.id,
        glId: this.data.fiberObj.id,
        glqx: this.detailsDataEditBox.glqx,
        gldId: this.detailsDataEditBox.gldId,
        gldqx: this.detailsDataEditBox.gldqx,
        ...
        if (res != '') {
          this.detailsData.data[this.detailsDataEditBox.index].ID = res
        }
        this.detailsData.data[this.detailsDataEditBox.index].gldqx = this.detailsDataEditBox.gldqx
        this.detailsDataEditBox.show = false

        getFiberCoreDetailApi({
          fiberId: this.data.fiberObj.id
        }).then(res => {
          this.data = res.data
        })
        this.locationLine()
