<template>
  <div class="app-container">
    ...
    <!-- 添加或修改流程定义对话框 -->
    <el-dialog
      v-dialogDrag :title="title" :visible.sync="open" width="500px" append-to-body :close-on-click-modal="false">
    ...
    <!-- bpmn20.xml导入对话框 -->
    <el-dialog
      v-dialogDrag :title="upload.title" :visible.sync="upload.open" width="400px" append-to-body :close-on-click-modal="false">
      <el-upload
        ref="upload"
        :limit="1"
        accept=".xml"
    ...
    <!-- 流程图 -->
    <el-dialog
      v-dialogDrag :title="readImage.title" :visible.sync="readImage.open" width="70%" append-to-body :close-on-click-modal="false">
    ...
    <!--表单配置详情-->
    <el-dialog
      v-dialogDrag :title="formTitle" :visible.sync="formConfOpen" width="50%" append-to-body :close-on-click-modal="false">
      <div class="test-form">
    ...
    <!--挂载表单-->
    <el-dialog
      v-dialogDrag :title="formDeployTitle" :visible.sync="formDeployOpen" width="60%" append-to-body :close-on-click-modal="false">
      <el-row :gutter="24">
...
export default {
  ...
  methods: {
    ...
    /** 跳转到流程设计页面 */
    handleLoadXml(row){
      this.$router.push({ path: '/flowable/definition-model/index',query: { deployId: row.deploymentId }})
    },
    /** 流程图查看 */
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除流程定义编号为"' + row.deploymentId + '"的数据项?', "警告", {
